...
window.addEvent('domready', () => {
    var //cache sort container
    container = document.id('container');						//used to track what elements to swap on click
    ...
    document.id('order').addEvent('click', event => {
		event.stop();
		alert(sorter.currentOrder);
	});

    document.id('reorderDOM').addEvent('click', event => {
		event.stop();
		sorter.rearrangeDOM();
    ...
    document.id('swap').addEvent('click', event => {
		event.stop();
		var elems = container.getChildren();
		sorter.swap(elems[0], elems[elems.length - 1]);
	});

    document.id('sort').addEvent('click', event => {
		event.stop();
		sorter.sort([1, 3, 0, 2, 4]);
	});

    $$('#fxTransition, #fxEase').addEvent('change', () => {
        var transition = document.id('fxTransition').get('value');
        var easing = document.id('fxEase').get('value');
